# Docs Overview

This `docs` folder contains the public-facing briefs and planning materials that are appropriate for a clean GitHub release.

## Contents

- `public-briefs/`: short public summaries of the candidate and project
- `FAQ.md`: short answers to common reviewer and collaborator questions
- `evidence/`: curated public evidence and compact result summaries
- `validation/`: the public validation scope and acceptance logic
- `funding/`: staged funding rationale for external support
- `workflows/`: public summary of the workflow surfaces

These files are intentionally narrower than the internal repository materials. They are designed to explain the project clearly without exposing private outreach operations or overstating experimental status.

Evidence in this public package is curated rather than exhaustive. The goal is to make the project legible and credible without dumping raw internal files or implying experimental proof.
