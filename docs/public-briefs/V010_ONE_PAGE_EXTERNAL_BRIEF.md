# V010 One-Page External Brief

## Request

V010 is an evidence-backed hydrogen-storage candidate that is ready for limited external validation. The immediate need is not a broad research program. It is a focused test campaign to determine whether a physical sample reproduces the modeled structure and room-temperature hydrogen behavior.

The most useful forms of support are:

1. modest funding for third-party validation testing
2. compute or cloud credits tied to the reporting workflow
3. an introduction to a qualified lab, academic group, or research partner

## Candidate Snapshot

V010 is derived from the base framework through two controlled changes:

- uniform framework expansion to a locked scale factor of 1.2600
- partial Cu to Ni substitution at 12 of 48 metal sites

This preserves the overall topology while testing a more open structure and adjusted node chemistry.

## Evidence So Far

The current package supports V010 as a serious validation candidate rather than a speculative concept.

- modeled hydrogen uptake at 298.15 K: about 5.03 wt%
- mean release: about 94.70%
- durability rerun: stable across 10 cycles
- temperature stress checks: stable from 273.15 K to 323.15 K
- 95/5 H2/N2 screen: hydrogen remained the dominant adsorbed component
- mass-transfer review: no apparent diffusion bottleneck

Internal proving gates were passed for the modeled campaign.

## External Validation Scope

The unresolved question is physical validation.

The first-stage checks are:

1. PXRD confirmation against supplied references
2. activation and sample-quality verification
3. room-temperature hydrogen uptake and release testing
4. short repeat-cycle confirmation

## Why This Is Worth Supporting

This is a credible AI-for-science test case because it has:

- a narrow scientific question
- a preassembled evidence package
- a defined go or no-go experiment
- useful outputs even if the physical result is negative

That last point matters. A negative result would still produce useful validation data on AI-assisted materials search and the gap between modeled and experimental outcomes.

## Positioning Line

"This is a small AI-supported discovery validation effort with clear milestones, a defined external test package, and useful outputs regardless of outcome."
